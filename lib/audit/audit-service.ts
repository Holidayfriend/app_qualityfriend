...
function descriptions(actor: string, entry: AuditEntry) {
  ...
  for (const locale of ["en", "de", "it"] as const) {
    ...
    else if (afterStatus === "PUBLISHED") result[locale] = locale === "de" ? `${actor} hat ${entity} „${after || before}“ veröffentlicht` : locale === "it" ? `${actor} ha pubblicato ${entity} “${after || before}”` : `${actor} published ${entity} “${after || before}”`;
    else if (afterStatus === "APPROVED") result[locale] = locale === "de" ? `${actor} hat ${entity} „${after || before}“ genehmigt` : locale === "it" ? `${actor} ha approvato ${entity} “${after || before}”` : `${actor} approved ${entity} “${after || before}”`;
    else if (afterStatus === "REJECTED") result[locale] = locale === "de" ? `${actor} hat ${entity} „${after || before}“ abgelehnt` : locale === "it" ? `${actor} ha rifiutato ${entity} “${after || before}”` : `${actor} rejected ${entity} “${after || before}”`;
