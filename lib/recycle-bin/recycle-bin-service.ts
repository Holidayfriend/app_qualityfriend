...
export type RecyclableType = "department" | "team";
const registry = {
  department: { table: "departments", entityType: "DEPARTMENT" },
  team: { table: "teams", entityType: "TEAM" },
} as const;
...
export async function listDeletedItems() {
  ...
  const groups = await Promise.all((Object.keys(registry) as RecyclableType[]).map(async (type) => {
    const result = await queryDatabase<DeletedRow>(`SELECT id, name_en, name_de, name_it, deleted_at FROM ${registry[type].table} WHERE hotel_tenant_id = $1 AND is_deleted = true ORDER BY deleted_at DESC`, [current.hotel_tenant_id]);
...
export async function restoreDeletedItem(type: RecyclableType, id: string) {
  ...
  try {
    ...
    const result = await client.query<DeletedRow>(`UPDATE ${registry[type].table} SET is_deleted = false, is_active = true, deleted_at = NULL, updated_by_id = $1, updated_at = NOW() WHERE id = $2 AND hotel_tenant_id = $3 AND is_deleted = true RETURNING id, name_en, name_de, name_it, deleted_at`, [current.id, id, current.hotel_tenant_id]);
