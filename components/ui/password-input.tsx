...
export function PasswordInput({ id, label, showLabel = "Show password", hideLabel = "Hide password", className = "", ...props }: PasswordInputProps) {
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="relative">
        <input id={id} type={visible ? "text" : "password"} className={`h-11 w-full rounded-lg border border-[var(--qf-border)] bg-white px-3.5 pr-11 text-sm text-[var(--qf-text)] outline-none transition placeholder:text-[var(--qf-text-light)] hover:border-[#d8d4cc] focus:border-[var(--qf-accent)] focus:ring-3 focus:ring-[var(--qf-accent-soft)] ${className}`} {...props} />
