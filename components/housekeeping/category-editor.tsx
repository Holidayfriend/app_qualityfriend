...
type Values = { name: string; expressMinutes: string; normalMinutes: string; departureMinutes: string; finalMinutes: string; cleaningFrequency: Frequency; cleaningWeekdays: number[]; linenFrequency: Frequency };
const empty: Values = { name: "", expressMinutes: "", normalMinutes: "", departureMinutes: "", finalMinutes: "", cleaningFrequency: "DAILY", cleaningWeekdays: [], linenFrequency: "DAILY" };
...
export function CategoryEditor() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/housekeeping/categories/${id}?locale=${locale}`, { cache: "no-store" }).then(async (response) => {
      ...
      return response.json() as Promise<{ category: { name: string; expressMinutes: number | null; normalMinutes: number | null; departureMinutes: number | null; finalMinutes: number | null; cleaningFrequency: Frequency | null; cleaningWeekdays: number[]; linenFrequency: Frequency | null } }>;
