...
import { createPaypalSubscription, getPaypalSubscription, paypalPlanId } from "../../../../../lib/billing/paypal";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const created = await createPaypalSubscription(user.hotelTenantId,amount);
    if (!created.id) throw new Error("PayPal returned an invalid subscription.");
    const paypal = await getPaypalSubscription(created.id);
    if (paypal.id !== created.id || paypal.plan_id !== await paypalPlanId() || paypal.custom_id !== user.hotelTenantId) throw new Error("PayPal returned an invalid subscription.");
