...
import { accessibleModules, type AccessUser } from "../../../lib/auth/module-access";

type CurrentUser = AccessUser & { first_name: string; last_name: string; language: "EN" | "DE" | "IT"; hotel_name_en: string; hotel_name_de: string; hotel_name_it: string };

export async function GET() {
  const userId = await getSessionUserId();
  if (!userId) return NextResponse.json({ error: "UNAUTHENTICATED" }, { status: 401 });
  const result = await queryDatabase<CurrentUser>(
    `SELECT u.id, u.hotel_tenant_id, u.first_name, u.last_name, u.role, u.language, h.hotel_name_en, h.hotel_name_de, h.hotel_name_it
  ...
  return NextResponse.json({ ...user, allowed_modules: await accessibleModules(user) });
}
