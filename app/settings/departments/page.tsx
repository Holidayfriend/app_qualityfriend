...
export default function DepartmentsPage() {
  ...
  function openModal(type: EntityType, index: number | null) {
    ...
    setName(entity?.names[locale] ?? "");
    setNameError(""); setRequestError(""); setModal({ type, index });
  }

  async function save(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!modal || saving) return;
    if (!name.trim()) return setNameError(c.required);
    const items = modal.type === "department" ? departments : teams;
    const edited = modal.index === null ? null : items[modal.index];
    setSaving(true); setRequestError(""); setNameError("");
    try {
