...
export default function TrainingPage() {
  ...
  const copy = locale === "de" ? { uploading: "Wird hochgeladen…", loading: "Trainingsdateien werden geladen…", deleteUnavailable: "Die Lösch-API ist noch nicht verfügbar.", invalid: "Nur JSON- und JSONL-Dateien sind erlaubt.", failed: "Trainingsdateien konnten nicht geladen werden." } : locale === "it" ? { uploading: "Caricamento…", loading: "Caricamento dei file di addestramento…", deleteUnavailable: "L’API di eliminazione non è ancora disponibile.", invalid: "Sono consentiti solo file JSON e JSONL.", failed: "Impossibile caricare i file di addestramento." } : { uploading: "Uploading…", loading: "Loading training files…", deleteUnavailable: "The delete API is not available yet.", invalid: "Only JSON and JSONL files are allowed.", failed: "Training files could not be loaded." };
  const [files, setFiles] = useState<TrainingFile[]>([]); const [notice, setNotice] = useState(""); const [toast, setToast] = useState(""); const [loading, setLoading] = useState(true); const [uploading, setUploading] = useState(false);
  ...
  async function upload(selected: File[]) { if (selected.some(file => !/\.(json|jsonl)$/i.test(file.name))) { setNotice(copy.invalid); return; } setUploading(true); setNotice(""); const form = new FormData(); selected.forEach(file => form.append("files", file)); try { const response = await fetch("/api/settings/training", { method: "POST", body: form }); const data = await response.json(); if (!response.ok) throw new Error(data.message || copy.failed); setNotice(t.uploadReady); await load(); } catch (error) { setNotice(error instanceof Error ? error.message : copy.failed); } finally { setUploading(false); } }
  return <AppShell activeItem="settings" pageTitle={t.trainingTitle}><main className="p-4 sm:p-5 lg:px-7 lg:py-6">
    ...
    <section className="rounded-[var(--qf-radius)] border border-[var(--qf-border)] bg-white p-4 shadow-[var(--qf-shadow)] sm:p-5"><FileDropZone label={uploading ? copy.uploading : t.uploadTraining} messages={t} onFiles={(selected) => { if (!uploading) void upload(selected); }} fileTypes=".json,.jsonl,application/json,application/jsonl" formats="JSON / JSONL" />
